...
class QueueListenerTest {
    ...
    void shouldThrowException_whenMessagesHandlerThrows() throws Exception {
        ...
        doThrow(RuntimeException.class).when(messagesHandler).handle(queueMessage, message, channel);
        doAnswer(invocation -> {
            Runnable runnable = invocation.getArgument(0);
            runnable.run();
            return null;
        ...
        assertThrows(RuntimeException.class, () -> queueListener.listen(queueMessage, message, channel));
        verify(threadPoolTaskExecutor).execute(any(Runnable.class));
    }
